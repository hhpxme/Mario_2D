...
public class WinState extends GameState {
	...
	public WinState(GameStateManager gsm) {
		...
		try {
			logo = ImageIO.read(new File("src/main/resources/Icon/game_winner.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void draw(Graphics2D g) {
		background.draw(g);
		g.drawImage(logo, GamePanel.WIDTH / 2 - logo.getWidth() / 2, GamePanel.HEIGHT / 4, null);
		...
		for (int i = 0; i < menu.length; i++) {
			...
			g.drawString(menu[i], GamePanel.WIDTH / 2 - g.getFontMetrics(selectionFont.deriveFont(Font.PLAIN, 14)).stringWidth(menu[i]) / 2, GamePanel.HEIGHT / 2 + 10 + i * 35);
		}
	}
